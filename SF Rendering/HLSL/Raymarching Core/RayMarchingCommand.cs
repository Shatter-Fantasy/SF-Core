...
namespace SF.Rendering.RayMarching
{
    ...
    /// <summary>
    /// Data struct for RayMarching commands that can be either sent to a compute shader for GPU based work
    /// or sent to a <see cref="RayMarchingStepJob"/> to use Parallel processing to calculate the job on a CPU.
    /// </summary>
    [Serializable]
    [BurstCompile]
    public struct RayMarchingCommand : IRayMarchingCommand
    {
        ...
        {
            using var stepPositions = new NativeArray<Vector3>((MaxSteps + 1),Allocator.TempJob);
        ...
        /* TODO: Implement this by finishing the JobHandle return after setting the parameters.
